...
**`--help` を先に見る。** plan の形も、`goto` `do` の意味も、`runtime` `inputs` の使い分けも、
...
`plan.json` に書き、`route.py flow --plan` に渡す。呼び出し元の `manifest.py` も
同じファイルから `title` と `expect` を拾う。**項目・経路・期待の元はこの1つだけ**で、
誰も写さない。
...
```json
{
  ...
  "items": [
    ...
    {"shot": "iphone_03_detail", "screen": "detail", "restart": true,
     "title": "行をタップすると詳細に移る",
  ...
  "explore": [
    {"shot": "iphone_04_history", "screen": "history",
     "title": "…", "expect": "…",
     "reason": "goto history: 画面 history がマップに無い"}
  ]
}
```
...
- **1項目＝ `steps` のあとに1枚。** `steps` は前の項目を撮った地点からの続きで、`{"goto": 画面id}` か `{"do": 操作id}` を並べる。何もせず同じ画面をもう1枚撮るなら `steps` は空
- **`shot` は証跡の名前。** パスではない（`shots_dir` と組み合わせて route.py が絶対パスにする）。項目・証跡・ダンプを結ぶ唯一の手がかりなので、**項目ごとに別の名前にする**
- **`runtime` / `inputs` は plan の頭に書く。** どの項目の操作にも効く（`goto` の経路の途中で叩く行にも）
...
**1項目1フローにしない。** 1本ごとに `stopApp` からやり直すので、起動と経路のぶんが項目数ぶん乗る。**連続して踏める項目は続けて並べ、`restart` は要るところにだけ付ける。** `restart` の付いた項目の手前でアプリを起動し直して起点に戻るので、前の項目の状態は引き継がれない。**長くすれば安くなるが、途中で落ちるとその `restart` までが全部撮れない。** 3〜4項目を目安にする。
...
- **画面をまたいで突き合わせる項目**（詳細の表示が一覧と一致しているか等）は、**`restart` を挟まずに続けて並べる。** 挟むと起動し直して一覧を取り直し、同じデータを見ている保証が消える
...
- **「確認項目」は plan.json を読み上げたもの。** 正は plan で、呼び出し元の `manifest.py` がそこから `title` と `expect` を拾う。**ここに書いて plan に無いものは、どこにも残らない**。直すなら plan を直す
...
- **フローは plan のパスと、`route.py flow --plan` の標準出力と生成先のパスで返す。** YAML の中身は貼らない。標準出力は走るファイルと同じ実行から出た操作列なので、呼び出し元はそれをそのままレビューの2段目に出し、sim-driver にはパスを渡す
