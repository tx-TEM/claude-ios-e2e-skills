...
```bash
python3 ~/.claude/skills/screen-map/scripts/route.py check --repo <アプリのリポジトリ>
```
...
このスキルの成果物は、このスキルの `scripts/route.py` が経路を組む入力になる。sim-test-report はそれを使って動作確認のフローを作る（`expect` の `screen` を辺にした最短路）。`anchor` と `ready` が到達判定に、`screen: back` が復路に、`in_tree: false` と条件つきの要素・枝が経路の切れ目になる。**書く側として効いてくる点と route.py の使い方は `reference/route.md`**（`python3 ~/.claude/skills/screen-map/scripts/route.py --help` も参照）。
