...
`flow` は **plan（項目と経路を書いた JSON）を読み、項目ごとにフローを分けて書く**。項目1つ＝ `steps` のあとに1枚撮る。`steps` は `goto <画面id>`（いま居る画面からそこまで計算して繋ぐ）と `do <操作id>`（その場で操作する）を並べたもので、**並び順がそのまま実行順**。形は `--help` にある。sim-test-report の test-case-builder が書く。
...
- **`text` の値はマップが持たない。** 呼ぶ側が plan の `inputs` か `runtime` で渡す。何を打つかを決めるのはテストケース
