...
`flow` は **plan（確認項目の JSON）を読み、項目ごとにフローを分けて書く**。項目1つ＝ `screen` で `do` の操作を順に叩いて1枚撮る。**項目は経路を持たない** — 前の項目が終わった画面から `screen` までは、ここがマップから計算して繋ぐ。形は `--help` にある。sim-test-report の test-case-builder が書く。
