...
同名のどれを押すかは**実行時の選び方**で、識別子に焼き込むのとは別物。マップにも書かない（どれを押すかはスクリプトが実行時に決める。`reference/route.md`）。スクリプトは押す直前の画面から、同じ ID の行のうち何番目かを数えて Maestro の `index` に渡すので、同名の行があっても狙った1件を押せる。
