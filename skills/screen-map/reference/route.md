...
- **`auto_shows` は入るたびに確かめる。** 並べた画面に入ると、その画面の anchor を待つ**前に**「自動表示の画面の anchor が出ていたら、その `kind: dismiss` を叩く」が積まれる（戻る操作で戻ったときは積まない）。自動表示が出ている間は、下の画面の anchor がツリーから隠れるので、この順番でないと落ちる。出ていないときに1つ約7秒かかるので、実際に出る画面にだけ並べる。自動表示の画面は遷移で入らないが、出る先に着けるなら `check` は到達できるものとして数え、項目の `from` にも書ける（出る先まで行って、閉じずに待つ）
