...
| `desc` / `result` | **手順2で埋める。** ここでは空と `未判定` のまま |

`build_report.py` は **`title` が空、または `result` が `未判定` のままなら止まる。** 骨組みのままレポートが出ることはない。
...
- **レイアウト崩れ・要素の重なり・色・ルビの描画は画像でしか分からない。** ダンプに行が出ない要素（テキストもidも持たないタップ領域、画像）も同じ。**この種の破綻は sim-driver の報告にも出てこない**ので、気づけるのは証跡を読む自分だけ
...
**マニフェストは手順0で作ってある。** 埋めるのは `desc` と `result` だけで、`title` / `expect` / `images` は触らない（合意した内容なので、判定の側で書き換えない）。

```json
"desc": "入力欄に「カラマゾフ」が入り、一覧が1件（カラマゾフの兄弟 / 01 上 / 中山 省三郎）に絞られた。",
"result": "OK"
```

- **`expect` と見比べて埋める。** 期待と違ったら `result` を `NG` にし、`desc` に何がどう違ったかを書く
- **`result` は `未判定` のままだと `build_report.py` が止まる。** 全項目を埋め切らないとレポートが出ない
- 撮り直しや項目の増減があれば、ここで `sections` を足し引きする
