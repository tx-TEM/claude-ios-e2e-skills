...
`route.py path` が出す。**accessibilityIdentifier で書かれた実際の操作列**で、どこを撮るかも入っている。
...
**整形した要約を別に作らない。** 要約を見せてフローを走らせると、レビューしたものと実際に走るものが別になる。2段目・3段目は `path` が同じ経路から出したもので、**走るのは `flow` のほう**。求められたらそのまま出す。
