...
`test-case-builder` が返した項目に、`route.py` の出力から**機械判定を項目ごとに付けて**出す。
...
**レビューが通った時点で `manifest.json` を作る。** レポートの直前ではなく、ここで。
...
- **`result` は省略できない。** 無ければ `build_report.py` がエラーで止まる。`未判定` が残っていればそのまま出るので、**全項目を埋め切る**
