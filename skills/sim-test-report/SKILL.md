...
python3 $R flow --app <bundle id> --map <アプリのリポジトリ> \
  --goto browse --shot <出力先>/iphone_01_list \
  --goto detail --shot <出力先>/iphone_02_detail \
  --out ~/.claude/skills/sim-test-report/.work/flow_A.yaml
```
...
**経路を添えた項目は、観測した事実も返らない。** フローの中に到達判定（`extendedWaitUntil`）が積んであるので、**通ったこと自体が「その画面に着き、期待した要素が出た」証明**になっている。そのうえで文章に起こさせると、PNGを読んで判定する内容と重複するだけで、しかも**ダンプはフローの途中では取れないので、そのために同じ道を歩き直すことになる**（実測で、8項目3フローの実行の8回中5回がその歩き直しだった）。探索で撮った項目は何も検証されていないので、そちらは事実が返る。
