...
```bash
python3 $R flow --app <bundle id> … --out-dir .work/flows/<slug>/A/
python3 <このスキルのディレクトリ>/scripts/manifest.py .work/flows/<slug>/A/index.json <出力先> \
...
**`--explore` は経路が組めなかった項目。** 索引に無いので名前だけ渡す。`flow` を持たないので `run_flows.py` は飛ばし、sim-driver が探索で撮る。**末尾に並ぶ。**

**埋めるのは `title` と `expect` だけ。** どちらも索引からは導けない — 何を確かめるかと、渡したデータで何が起きるか。**`test-case-builder` が返した項目と期待をここに入れる。**
...
**順番を気にしなくてよい。** `run_flows.py` が1グループを一息に走らせ切ってから sim-driver に渡すので、フローの間に探索が入ることがない。マニフェストの並びは**レポートで見せたい順**のままでよい。

#### フローのある項目

```bash
python3 <このスキルのディレクトリ>/scripts/run_flows.py \
  <出力先>/manifest.json .work/flows/<slug>/A <UDID>
```
...
- **落ちたらそこで止まる。** フローは前のフローの続きなので、1本落ちたあとを走らせても意味がない。**落ちた地点の画面が出る**ので、それを読んで直す
