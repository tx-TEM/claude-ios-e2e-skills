...
**撮影は Maestro を既定とする。** LLMが画面を見て操作するのは、Maestro が扱えない区間だけ。テストケースが固まった時点で、**どの区間がそれに当たるか**を見ておく。

まず `shared_dirs`（解決のしかたは手順1）にそのアプリのフローがあるか見る。あれば `README.md` を読み、用意されている導線と、そのアプリで Maestro が動かない画面を把握する。

LLMに回すのは次の区間。

- **大量のデータを持つ一覧画面を通るところ。** Maestro はアクセシビリティの階層を読んで操作するため、階層が大きいと取得自体に失敗する
- **導線がまだ分かっていないところ。** 何をタップすればいいかが確定していない区間は、LLMが通して確定させる

それ以外は Maestro で書ける。フローが無くてもよく、その場で書いて走らせる（`references/maestro-flow.md`）。
