...
#### 撮り終えたら、通した導線をフローに残すか決める

sim-driver が自分で操作して撮った導線は、その時点で**確定している。** またすぐ撮ることになりそうなら、フローにして残す。

ここを飛ばすと、苦労して見つけた導線がセッションと一緒に捨てられ、次回また同じ探索をやり直すことになる。フローは最初から書けるものではなく、**一度LLMが通した導線を固めたもの**として増えていく。

残す価値の基準は手順0と同じ。次回以降も通る導線、汎用的な導線、階層が深い区間。置き場所も同じで、汎用的なものは `common/`、その確認に固有のものはケース側へ。

**フローはスクリーンショットから起こさない。** sim-driver の報告から分かるのは「どの画面をどの順で通ったか」までで、そこに写っていた座標やラベルをそのままセレクタにすると壊れやすい。画面から読み取った座標でタブを叩いたつもりが別の要素に当たる、ということが起きる。安定した識別子はコードにしか書かれていない。**確定した導線を手がかりに、コードでセレクタを裏取りしてから書く**（`references/maestro-flow.md`）。

作成にはアプリのソースを読む作業が要るので、手順0と同じく**ユーザーに聞いてから**始める。レポートを先に届けて、フロー化は別途でもよい。
