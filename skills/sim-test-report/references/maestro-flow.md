...
`config.json` の `flow_dirs` を上から見て、最初に存在したディレクトリを使う。既定はこの順。

1. **`<アプリのリポジトリ>/maestro/`** — こちらが本来の置き場所。フローはアプリの実装に紐づくので、
   セレクタの出どころと同じリポジトリで版管理し、チームで共有できる。
   アプリ側にコミットが必要になるが、UIを変えた人がフローも直せる状態にしておく価値のほうが大きい
2. **`<このスキル>/.cache/<アプリのリポジトリ名>/`** — 1が無いときの退避先。
   gitignore 済みで端末ローカル。他の端末には運ばれないので、消えても作り直せる前提で使う

アプリのリポジトリに置けるなら1にする。2は、アプリ側にコミットを増やしたくない段階や、
試している最中の置き場所として使う。
