...
テキストは壊れやすい。ローカライズで変わり、件数や記号が付き、同じ文言が複数ある。
`accessibilityIdentifier` はそのどれにも影響されない。

- ローカライズされないので、文言を変えても壊れない
- 画面に出ないので、デザイン変更の影響を受けない
- **VoiceOver は読まない。** `accessibilityLabel` と違い、付けても実際のアクセシビリティ体験は
  変わらない。テスト用として足せる
...
**ただし全面的に振らない。** アプリ側のコード変更なのでチームの合意が要るし、使われない
識別子が増えるだけになる。**実際に壊れた場所にだけ足す。** 撮影で落ちた記録が
`README.md` に溜まっていくので、それがそのまま候補のリストになる。

**付けても効かない要素がある。** `UIBarButtonItem` のようにビューを直接持たないものは、
識別子が内部のビューに伝わらず階層に現れないことがある。その場合は画像のアセット名など、
実際に階層へ出ている値を使う。何が出ているかはビュー階層で確かめる。
...
**テキストセレクタは部分一致しない。全文一致の正規表現として扱われる。**

画面に「予定（2）」と出ているとき、`予定` では**一致しない。** 件数や記号が付く文言、
末尾が変わる文言は素直に書くと必ず外れる。前後を `.*` で囲む。
...
**同じ文言の要素が複数あると、意図しないほうに当たる。**
タブ名のつもりで書いた文字列が、本文中の同じ文言に当たって別の画面が開くことがある。
`id:` があるならそちらを優先し、無ければ `index:` で絞る。
...
`--test-output-dir` を指定すると、実行のたびに `<日時>/` が積まれる。中にはステップごとの
スクリーンショット、失敗時の画面階層、ログが入る。

**撮り終えた直後には消さない。** 失敗の原因を後から追うのはこの成果物で、
撮り直しの判断をするときにも要る。

代わりに、**実行のはじめに古い出力を消す。** 出力先を作業用のディレクトリにしておき、
前回までのものを落としてから走らせる。溜め続けなければ量は問題にならない。
...
証跡そのものは、この出力先から証跡ディレクトリへコピーした時点で独立している。
出力先を消しても証跡は残る。
...
- そのアプリで踏んだ落とし穴
- **Maestro が動かない画面。** 撮影で落ちるたびに書き足していく。ここに載っていれば、次回は
  試さずに最初からLLMへ回せる。落ちるまでに数十秒かかるので、記録しないと毎回それを捨てる。
  何をしようとして、どのエラーで落ちたかまで書く
